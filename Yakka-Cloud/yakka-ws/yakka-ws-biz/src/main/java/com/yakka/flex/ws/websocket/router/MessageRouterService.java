package com.yakka.flex.ws.websocket.router;
...
/**
 * 消息中转工具
 * 功能：其他没有依赖yakka-router的服务，比如oauth服务; 需要将消息推送给任何一个用户就需要先将消息推送到当前消费者，再由当前消费者将消息推送到目标 uidList 所在的ws节点
 ...
 * 动态路由: 使用 Redis 存储 com.yakka.flex.router.RouterCacheKeyBuilder.DeviceNodeMapping({uid}:{clientId}) 映射关系, NacosRouterService.findNodeDeviceUser() 批量查询用户所在节点
