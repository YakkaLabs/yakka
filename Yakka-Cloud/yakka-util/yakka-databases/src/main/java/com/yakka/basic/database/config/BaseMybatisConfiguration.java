package com.yakka.basic.database.config;
...
import com.yakka.basic.database.injector.YakkaSqlInjector;
import com.yakka.basic.database.mybatis.typehandler.FullLikeTypeHandler;
import com.yakka.basic.database.mybatis.typehandler.LeftLikeTypeHandler;
import com.yakka.basic.database.mybatis.typehandler.RightLikeTypeHandler;
import com.yakka.basic.database.properties.DatabaseProperties;
import com.yakka.basic.uid.dao.WorkerNodeDao;
...
/**
 * Mybatis 常用重用拦截器，yakka.database.multiTenantType=任意模式 都需要实例出来
 * <p>
 * 拦截器执行一定是：
...
public abstract class BaseMybatisConfiguration {
    ...
    public MetaObjectHandler getMyMetaObjectHandler() {
        return new YakkaMetaObjectHandler();
    }
    ...
    /**
     * yakka.database.id-type = DEFAULT 或 yakka.database.id-type 未设置时启用。
     */
    @Bean
    @ConditionalOnMissingBean
    ...
    /**
     * yakka.database.id-type = CACHE 时启用。
     */
    @Bean
    @ConditionalOnMissingBean
    ...
    /**
     * yakka.database.id-type = HU_TOOL 时启用。
     */
    @Bean
    @ConditionalOnMissingBean
    ...
    @Bean
    @ConditionalOnMissingBean
    public YakkaSqlInjector getMySqlInjector() {
        return new YakkaSqlInjector();
    }
